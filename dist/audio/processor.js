...
function makeDeck(){return {id:null,pos:0,playing:false,speed:1,pitch:1,keyLock:false,reverse:false,loop:false,start:0,end:0,slip:false,ghost:0,scratch:false,scratchRate:0,gain:0,grainPhase:0,g1:0,g2:0,reset:true,roll:false,rollGhost:0,rollState:null};}
class OrbitProcessor extends AudioWorkletProcessor{
 constructor(){super();this.tracks=new Map();this.decks=Array.from({length:4},makeDeck);this.voices=Array.from({length:16},()=>({on:false,slot:-1,id:null,pos:0,end:0,start:0,gain:1,loop:false}));this.frames=0;this.report=0;this.port.onmessage=e=>this.command(e.data);}
 command(m){
 ...
 align(t,d,candidate,old,pitch){let best=candidate,score=Infinity;for(let off=-256;off<=256;off+=8){let sum=0;for(let k=0;k<192;k+=8){const a=this.read(t,0,old+k*pitch,d),b=this.read(t,0,candidate+off+k*pitch,d),v=a-b;sum+=v*v;}if(sum<score){score=sum;best=candidate+off;}}return best;}
 process(_,outputs){
  const len=outputs[0]?.[0]?.length||128;
  for(let di=0;di<4;di++){
   const d=this.decks[di],t=this.tracks.get(d.id),out=outputs[di];if(!out?.[0]||!t)continue;const l=out[0],r=out[1]||out[0],ratio=t.sr/sampleRate,dir=d.reverse?-1:1;let speed=(d.scratch?d.scratchRate:d.speed*dir)*ratio;const pitch=(d.keyLock?d.pitch:d.speed*d.pitch)*dir*ratio;const granular=!d.scratch&&(d.keyLock||Math.abs(d.pitch-1)>.0001)&&Math.abs(pitch-speed)>.0001;
   ...
   for(let i=0;i<len;i++){
    ...
    if(d.playing&&d.scratch)d.ghost+=d.speed*dir*ratio;if(d.playing&&d.roll)d.rollGhost+=d.speed*dir*ratio;
  ...
  this.report+=len;if(this.report>=sampleRate/25){this.report=0;this.port.postMessage({type:'positions',positions:this.decks.map(d=>d.pos/(this.tracks.get(d.id)?.sr||sampleRate)),playing:this.decks.map(d=>d.playing),samples:this.voices.filter(v=>v.on).map(v=>v.slot)});}
  return true;
 }
}

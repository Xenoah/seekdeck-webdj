...
export function planExchange(parsed,tracks){
 ...
 for(const group of groups.values())if(new Set(group.map(r=>{const p=normalizePath(r.source.path);return /^[a-z]:\//i.test(p)?p.toLowerCase():p;})).size>1)for(const row of group)row.status='ambiguous';

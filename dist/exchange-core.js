...
export function parseCSV(text,name){
 ...
 for(const key of ['cueDetails','memoryCues','tempoMap','gridMarkers']){const raw=pick(r,key);if(raw){let value;try{value=JSON.parse(str(raw));}catch{throw new Error(`CSVの${key}が不正なJSONです。`);}if(!Array.isArray(value))throw new Error(`CSVの${key}は配列にしてください。`);t[key]=value;}}
...
export function writeCSV(tracks,options){const header=['seekdeck_csv','path','title','artist','album','genre','bpm','key','comment','duration','grid_offset','rating',...Array.from({length:8},(_,i)=>'cue'+(i+1)),'cueDetails','memoryCues','tempoMap','gridMarkers'];return '\uFEFF'+[header.join(','),...tracks.map(t=>[1,exportPath(t,options),t.name,t.artist,t.album,t.genre,t.bpm,t.key,t.comment,t.duration,t.gridOffset,t.rating,...Array.from({length:8},(_,i)=>t.cues?.[i]??''),JSON.stringify(t.cueDetails||[]),JSON.stringify(t.memoryCues||[]),JSON.stringify(t.tempoMap||[]),JSON.stringify(t.gridMarkers||[])].map(safeCell).join(','))].join('\r\n')+'\r\n';}

...
幅859px以下（タッチ端末の横画面は1100px以下）では専用UIになります。上部で操作するデッキA〜D、下部でデッキ・ミキサー・曲・サンプルを選択します。曲一覧の「A〜D へロード」は上部で選んだデッキが対象です。ミキサーはA/BまたはC/Dを表示します。スマホの選択はSYNCのMASTERやPCの保存配置を変更しません。

ホットキューの削除は「キュー削除」、サンプル編集は「パッドを編集」を有効にしてからパッドをタップします。画面切り替えで編集モードを解除します。縦横画面とセーフエリアに対応していますが、実機の音声出力・MIDI/HID対応はブラウザと端末に依存します。

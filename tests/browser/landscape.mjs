...
export async function runLandscape(browser,url){
 ...
 try{
  ...
  await tap('[data-mobile-surface=pads][data-deck="0"]');
  ...
  await waitForAsync(page,async()=>{const {api}=await import('/seekdeck-webdj/app.js');return api.getTrack(0).cues[7]==null;});
  await tap('[data-mobile-surface=tools][data-deck="1"]');
  ...
  assert.equal(await page.locator('[data-mobile-deck="1"]').getAttribute('aria-pressed'),'true');
  ...
  await tap('[data-mobile-view=sampler]');await page.screenshot({path:'test-results/mobile-landscape-sampler.png'});
  await tap('[data-mobile-deck="0"]');await tap('[data-mobile-view=deck]');
